...
### Patients who got censored before the first event does not affect KM and Cox PH analysis

df1 = tte_data_gen(N = 1e2, accrual = 6, tmed.true = 10, cmed.true = 5, dmed = 20)

df1 %>% filter(`Event Flag` == 1) %>% summarise(min(`Time from Rand. to Event/Censor`))

df2 = df1 %>% summarise(PFS = `Time from Rand. to Event/Censor`, EVN = `Event Flag`, Arm = Arm)

df3 = data.frame(PFS = rep(0.05, 1e2), EVN = rep(0, 1e2), Arm = c(rep("Treatment", 50), rep("Control", 50))) # before 1st event
df3 = data.frame(PFS = rep(5, 1e2), EVN = rep(0, 1e2), Arm = c(rep("Treatment", 50), rep("Control", 50))) # after 1st event

df4 = rbind(df2, df3)

fit2 = survfit(Surv(PFS, EVN) ~ Arm, data = df2)
ggsurvplot(fit2, risk.table = T, surv.median.line = "hv", xlim = c(0, 25), break.time.by = 5)

fit4 = survfit(Surv(PFS, EVN) ~ Arm, data = df4)
ggsurvplot(fit4, risk.table = T, surv.median.line = "hv", xlim = c(0, 25), break.time.by = 5)

coxph(Surv(PFS, EVN) ~ Arm, data = df2, method = "efron") %>% summary
coxph(Surv(PFS, EVN) ~ Arm, data = df4, method = "efron") %>% summary

logrank = survdiff(Surv(PFS, EVN) ~ Arm, data = df2)
logrank = survdiff(Surv(PFS, EVN) ~ Arm, data = df4)
pnorm(-sqrt(logrank$chisq))*2
